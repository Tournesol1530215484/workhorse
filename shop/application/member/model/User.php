...
class User extends Model
{
    ...
    public function login($data,$type=0,$backAct){
        ...
        $userData['password']=md5($data['password']);    //稍后做加密处理
        //验证数据库有没有这个数据
        $res=db('user')->where(array('username'=>$userData['username']))->whereOr(array('email'=>$userData['username']))->whereOr(array('phone_num'=>$userData['username']))->find();
        if($res){
